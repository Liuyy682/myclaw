...
describe('MyClaw WebUI', () => {
  ...
  test('opens the read-only memory drawer and switches between memory sections', async () => {
    ...
    const memoryTrigger = screen.getByRole('button', { name: /查看记忆/ })
    await user.click(memoryTrigger)
    ...
    expect(screen.queryByRole('dialog', { name: '长期记忆' })).not.toBeInTheDocument()
    expect(document.activeElement).toBe(memoryTrigger)
  })
  ...
  test('opens monitoring, shows metrics, and displays a trace waterfall', async () => {
    ...
    expect(traceTrigger).toHaveTextContent('成功')
    await user.click(traceTrigger)
    expect(await screen.findByText('llm.complete')).toBeInTheDocument()
    expect(screen.getByRole('dialog', { name: 'Trace 详情' })).toBeInTheDocument()
    await user.keyboard('{Escape}')
    expect(screen.queryByRole('dialog', { name: 'Trace 详情' })).not.toBeInTheDocument()
    expect(document.activeElement).toBe(traceTrigger)
  })
})
